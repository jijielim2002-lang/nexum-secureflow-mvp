...
// ─── Importer-focused type list ───────────────────────────────────────────────

const IMPORTER_TYPES: CashflowType[] = [
  "Supplier Advance", "Supplier Balance", "Duty / Tax",
  "Logistics Fee", "Insurance", "Customer Collection",
  "Inventory Cost", "Receivable", "Payable",
  "Nexum Held Amount", "Nexum Release Expected",
];

const ALL_TYPES_REST: CashflowType[] = [
  "Cash Inflow", "Cash Outflow", "Carrier Payment",
  "Haulier Payment", "Warehouse / Storage",
  "Claim Reserve", "Refund", "Other",
];
...
export default function CustomerCashflowPage() {
  ...
  const [form, setForm] = useState({
    cashflow_type:       "Supplier Advance" as CashflowType,
    amount:              "",
    currency:            "RM",
    expected_date:       "",
    description:         "",
  ...
  useEffect(() => {
    ...
    if (!profile || !profile.company_id) { setPageState({ status: "auth_error" }); return; }
    void loadItems();
  ...
  async function handleAddItem(e: React.FormEvent) {
    e.preventDefault();
    if (!profile?.company_id || !form.amount || isNaN(Number(form.amount))) {
  ...
  if (pageState.status === "loading") {
    return <div className="min-h-screen bg-[#0a0f1e] flex items-center justify-center"><p className="text-slate-500">Loading…</p></div>;
  }
  if (pageState.status === "auth_error") {
    return <div className="min-h-screen bg-[#0a0f1e] flex items-center justify-center"><p className="text-slate-400">Account not linked to a company. Contact your admin.</p></div>;
  }
  if (pageState.status === "error") {
    return <div className="min-h-screen bg-[#0a0f1e] flex items-center justify-center"><p className="text-red-400">{pageState.message}</p></div>;
  }
      ...
      <header className="sticky top-0 z-40 border-b border-slate-800/80 bg-[#0a0f1e]/90 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3">
          <div className="flex items-center gap-3">
            <Link href="/customer" className="text-xs text-slate-500 hover:text-slate-300">← Dashboard</Link>
            <span className="text-slate-700">/</span>
            <span className="text-xs font-semibold text-blue-400">Cash Flow</span>
          </div>
          <div className="flex items-center gap-3">
            <span className="text-xs text-slate-500">{profile?.company_name}</span>
            <LogoutButton />
          </div>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-6xl space-y-6 px-4 py-8">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-0.5 text-sm text-slate-400">{profile?.company_name} · Importer / Buyer view</p>
          </div>
        ...
        <div className="rounded-xl border border-amber-500/20 bg-amber-950/20 px-4 py-3 text-xs text-amber-300">
          <strong>Importer / Buyer view.</strong> Track your supplier advances, duty/tax
          obligations, logistics fees, inventory costs, and expected customer collections.
          Items marked <span className="rounded border border-blue-500/30 bg-blue-500/15 px-1">Nexum-controlled</span>{" "}
          are within Nexum workflow. Add external payments manually to see your full funding gap.
        </div>
        ...
        {showAddForm && (
          ...
          >
            <h2 className="mb-3 text-sm font-semibold text-emerald-400">Add Cash-Flow Item</h2>
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
              <div>
                <label className="mb-1 block text-[11px] text-slate-400">Type *</label>
                <select value={form.cashflow_type}
                ...
                >
                  {[...IMPORTER_TYPES, ...ALL_TYPES_REST].map((t) => <option key={t} value={t}>{t}</option>)}
                </select>
              </div>
              <div>
              ...
              <div>
                ...
                <input type="text" value={form.currency}
                  ...
                  placeholder="RM" className="w-full rounded-lg border border-slate-700 bg-slate-900 px-3 py-2 text-xs text-slate-200"
                />
              </div>
              <div>
              ...
              <div>
                ...
                <select value={form.status} onChange={(e) => setForm((f) => ({ ...f, status: e.target.value as CashflowStatus }))}
                  className="w-full rounded-lg border border-slate-700 bg-slate-900 px-3 py-2 text-xs text-slate-200"
                >
              ...
              <div className="sm:col-span-2">
                ...
                <input type="text" value={form.description} placeholder="e.g. 30% supplier advance to ABC Supplier"
          ...
          <div className="rounded-xl border border-red-500/30 bg-red-950/20 px-5 py-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-[11px] text-slate-500 mt-0.5">
                Supplier advances and import costs may exceed expected collection before goods are sold.
              </p>
            </div>
              ...
              <table className="w-full text-xs">
                ...
                <tbody className="divide-y divide-slate-800">
                  {filteredItems.map((item) => {
                    ...
                    return (
                      <tr key={item.id}>
                        <td className="py-2.5 pr-3 font-mono text-slate-400">{item.expected_date ?? "—"}</td>
