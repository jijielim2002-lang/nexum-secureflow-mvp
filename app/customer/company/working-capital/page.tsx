...
export default function CustomerWorkingCapitalPage() {
    ...
    <div className="min-h-screen bg-[#0a0f1e] text-white">
      <div className="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl px-4 py-3 text-sm text-amber-300">
          <strong>For Importers & Buyers:</strong> Detected gaps typically include supplier advance and balance obligations, duty/tax funding requirements before customs clearance, and inventory financing needs.
          These are working capital gap estimates only — not credit approvals or financing commitments.
        </div>
        ...
        <div className="bg-slate-800/60 border border-slate-700/60 rounded-2xl overflow-hidden">
          {loading ? (
            ...
            <div className="py-20 text-center">
              ...
              <p className="text-slate-600 text-sm">Needs are detected automatically by the Nexum system based on your trade and procurement activity.</p>
            </div>
          ) : (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-700/60 text-slate-400 text-left">
                    ...
                    <th className="px-4 py-3 font-medium">Job / Procurement</th>
                  ...
                  {needs.map((n) => (
                    <tr key={n.id} className="hover:bg-slate-700/20 transition-colors">
                      ...
                      <td className="px-4 py-3 text-xs text-slate-400 max-w-[220px]">
                        <span className="line-clamp-2">{n.recommended_next_action ?? n.rationale ?? "—"}</span>
                      </td>
                    </tr>
                  ))}
                </tbody>
