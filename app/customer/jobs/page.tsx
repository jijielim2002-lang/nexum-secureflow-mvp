...
export default function CustomerJobsPage() {
  ...
  useEffect(() => {
    ...
    supabase
      .from("secured_jobs")
      .select("job_reference, service_provider, service_type, route, currency, job_value, payment_status, job_status, current_milestone")
  ...
  const pendingAction  = jobs.filter(
    (j) =>
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans flex flex-col">
      ...
      <main className="mx-auto w-full max-w-7xl px-6 py-10">
        ...
        <div className="mb-8 flex items-start justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-slate-50">My Job Requests</h1>
            <p className="mt-1 text-sm text-slate-400">
              ...
              {state.status === "success" && (
                <>
                  ...
                  {formatValue(totalValue, "RM")} total committed value
                </>
              )}
            </p>
          </div>
        ...
        {state.status === "success" && pendingAction > 0 && (
          <div className="mb-6 flex items-start gap-3 rounded-xl border border-amber-500/20 bg-amber-500/5 px-5 py-4">
            <span className="mt-0.5 text-base">⚠</span>
            <div>
              <p className="text-sm font-semibold text-amber-300">
                {pendingAction} job{pendingAction > 1 ? "s" : ""} need{pendingAction === 1 ? "s" : ""} your attention
              </p>
              <p className="text-xs text-slate-400">
                Accept the job terms or submit payment to keep things moving.
              </p>
            </div>
          </div>
        )}
        ...
        {state.status === "loading" && (
          <div className="rounded-xl border border-slate-800 bg-slate-900/40 py-20 text-center">
            <div className="mb-4 inline-block h-7 w-7 animate-spin rounded-full border-2 border-emerald-500 border-t-transparent" />
          ...
          <div className="overflow-x-auto rounded-xl border border-slate-800">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-800/60">
                {jobs.length === 0 && (
                  <tr>
                    <td colSpan={8} className="px-4 py-16 text-center text-sm text-slate-600">
                      No secured jobs assigned to your account yet.
                    </td>
                  </tr>
                )}
                {jobs.map((job) => {
              ...
              {jobs.length > 0 && (
                <tfoot>
                  <tr className="border-t border-slate-800 bg-slate-900 text-xs text-slate-500">
                    <td colSpan={4} className="px-4 py-3 font-medium">Total committed value</td>
