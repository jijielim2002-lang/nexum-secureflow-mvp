...
export default function ProviderTradeFlowPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const res  = await fetch("/api/tradeflow", { headers: { Authorization: `Bearer ${token ?? ""}` } });
  ...
  const active = requests.filter(r => !["Closed","Cancelled"].includes(r.payment_status)).length;
        ...
        <div className="mb-8 flex items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="rounded-full border border-blue-500/30 bg-blue-500/10 px-2 py-0.5 text-xs text-blue-400 font-medium">Beta</span>
            </div>
            <p className="text-sm text-slate-400">
              Agent remittance · overseas partner payments · duty settlement · document-based release
            </p>
            {!loading && !error && (
              <p className="mt-1 text-xs text-slate-500">{active} active request{active !== 1 ? "s" : ""}</p>
            )}
          </div>
          <Link
            href="/provider/tradeflow/new"
        ...
        {!loading && !error && requests.length === 0 && (
          <div className="rounded-xl border border-slate-800 bg-slate-900/20 py-24 text-center">
            ...
            <p className="mt-2 text-sm text-slate-500 max-w-sm mx-auto">
              Use TradeFlow to coordinate overseas agent payments, remit customs duties, manage partner payments with document control.
            </p>
            <Link
              href="/provider/tradeflow/new"
        ...
        {!loading && !error && requests.length > 0 && (
          <div className="space-y-3">
            {requests.map(r => (
              <Link
                key={r.id}
                href={`/provider/tradeflow/${r.tradeflow_reference}`}
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="mt-1 flex items-center gap-4 text-xs text-slate-500">
                      ...
                      {r.supplier_country && <span>{r.supplier_country}</span>}
                      {r.payment_stage && <span>{r.payment_stage}</span>}
                    </div>
                  </div>
                  <div className="text-right shrink-0">
                    <p className="text-sm font-bold text-slate-100">{formatTradeAmount(r.requested_payment_amount, r.currency)}</p>
