-- =============================================================================
-- 020_cip_comprehensive_schema.sql
-- Comprehensive schema update for company_intelligence_profiles.
-- Supersedes migrations 015, 018, 019.  Safe to re-run — all operations are
-- conditional (ADD COLUMN IF NOT EXISTS, DROP CONSTRAINT IF EXISTS, etc.).
-- =============================================================================

-- ── 1. Add all columns that may be missing ────────────────────────────────────
ALTER TABLE public.company_intelligence_profiles
  ...
  -- Performance scores (US + UK spelling both present for compatibility)
...
-- ── 2. UNIQUE constraint on company_id (required for ON CONFLICT upsert) ──────
DO $$
BEGIN
  IF NOT EXISTS (
...
-- ── 3. risk_level CHECK (must include 'Not Available') ───────────────────────
ALTER TABLE public.company_intelligence_profiles
...
-- ── 7. cip_columns() — returns actual column list for API payload filtering ──
...
-- =============================================================================
-- END 020_cip_comprehensive_schema.sql
-- =============================================================================
